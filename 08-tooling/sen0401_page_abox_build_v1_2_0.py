...
N = sys.argv[1]; NUM = N; N = ("ch%s" % N) if N.isdigit() else N; REPO = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
UNITL = ("chapter %d" % int(NUM)) if NUM.isdigit() else json.load(open(sorted(__import__("glob").glob(os.path.join(REPO, "08-tooling", "%s-page" % N, "unit_v*.json")))[-1]))["label"]
rec = json.load(open(os.path.join(REPO, "08-tooling", "%s-page" % N, "build_record_v%s.json" % PV)))
OBJ = json.load(open(latest_input(os.path.join(REPO, "08-tooling", "%s-page" % N), "objectives")))
...
tr = json.load(open(os.path.join(REPO, "08-tooling", "%s-page" % N, "test_results_abox_v%s.json" % PV)))
...
for k, (text, lvl) in OBJ.items():
    L.append('pg:%s a ipo:LearningObjective, owl:NamedIndividual ; rdfs:label "%s"@en ; ld:bloomLevel ld:%s %s .' % (k, q(text), lvl, (' ; sen0401:servesOutcome ' + ', '.join('<http://example.org/sen0401#%s>' % o for o in SERVES)) if SERVES else ''))
