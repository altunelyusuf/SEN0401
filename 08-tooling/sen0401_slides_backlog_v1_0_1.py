...
ex:Finding_PagesOnSharedTemplate942 a backlog:RetrospectiveFinding ;
    ...
    backlog:hasRootCause "With SEN0414's pages accepted, the owner asked for SEN0401's pages to be built the same way. They were on version 3 of the template, without the agents' toolkit and conversations, background threads, visualisations, the grouped menu, responsive and scalable layout, text-size control, breadcrumb, explorer synchronisation or the antivirus hardening." ;
    backlog:hasRemedy "Chapter 1, chapter 2 and the Bitcoin in numbers page rebuilt as page version 9.4.2 from course_page_template_v9_4_2.html, byte-identical to SEN0414's, with SEN0401's text in its course configuration 2.0.0. Their agents search the unit's ontology, document and research record and the page itself: SEN0401 has no textbook ontology for Mastering Bitcoin and no course ontology yet, and the pages say so. The numbers page's nine charts are now created only when each first comes into view: drawing them all at load had blocked the page for 443 ms in one task, over the 200 ms budget; after the change no task at load, and 109 to 140 ms when a chart first shows. SEN0401's page tools were brought level with SEN0414's - page data 2.1.0 with evaluation steps, build 4.0.0 with the embedded corpus and mission, record 2.1.0 and ABox builder 1.2.0 for named units and charts - and its tests ported as 08-tooling/sen0401_page_test_v9_4_2.py, drawing their guide and follow-up questions from each unit's own concepts. At 2026-09-26T14:27:29 every check passed on all three pages: 52, 36 and 23 widgets, the nine charts drawn, 0 console errors, 0 WCAG 2 AA violations, no main-thread task over 200 ms; Stage 4 gates passed; a stale-value fixture refused on each." .

ex:Finding_EveryFileVersioned a backlog:RetrospectiveFinding ;
    rdfs:label "Every SEN0401 file versioned, with its version declared inside"@en ;
    backlog:belongsToLineage ex:Lineage ; backlog:hasFindingScope backlog:Scope_Methodology ;
    backlog:hasRootCause "The configuration ontology's EngineeringArtefact rule - every artefact's file name embeds its version, matching the version declared inside it - applied here as in SEN0414. SEN0401 had 13 files without a version in their name (both renewed decks, all deck tooling, the chapter list) and no tool, input or deck declaring its version inside." ;
    backlog:hasRemedy "The versioning sweep 1.0.1 renamed and declared them, earlier contents counted as MINOR steps and the added declaration as a PATCH; page inputs versioned when the pages were rebuilt; the five names git, GitHub and the OE publisher fix left untouched by the owner's ruling of 2026-09-26. The register's own evidence keeps the file names that were true when it was checked (L-112)." .
